...
public:
	...
	explicit vector2(float v) : x(T(v)), y(T(v)) {}
	...
	vector2 Normalized() const { const T invlen = std::sqrt(x*x + y*y); return vector2(x/invlen, y/invlen); }
	vector2 Normalized(T& length) const { length = sqrt(x * x + y * y); return vector2(x / length, y / length); }
