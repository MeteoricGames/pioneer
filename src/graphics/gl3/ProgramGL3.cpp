...
struct Shader {
	Shader(GLenum type, const std::string &filename, const std::string &defines) {
		...
		RefCountedPtr<FileSystem::FileData> logzCode = FileSystem::gameDataFiles.ReadFile("shaders/gl3/logz.glsl");
		assert(logzCode);
		RefCountedPtr<FileSystem::FileData> libsCode = FileSystem::gameDataFiles.ReadFile("shaders/gl3/lib.glsl");
		assert(libsCode);
		...
		if (s_bDumpShaderSource) {
			...
			const char SHADER_GL3_OUT_DIR_NAME[] = "shaders/gl3";
			FileSystem::userFiles.MakeDirectory(SHADER_OUT_DIR_NAME);
			FileSystem::userFiles.MakeDirectory(SHADER_GL3_OUT_DIR_NAME);
...
{
	const std::string filename = std::string("shaders/gl3/") + name;
